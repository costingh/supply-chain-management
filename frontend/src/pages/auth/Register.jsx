...
function Register() {
    ...
    const highlightLabel = (step, fieldName) => {
        let errorColorCode = '#d32f2f'
        let normalColorCode = '#455261'
        setStepNumber(step)
        // highlight the field
        const fieldColor = document.querySelector('.' + fieldName)
        fieldColor.style.color = errorColorCode
        setTimeout(() => {
            fieldColor.style.color = normalColorCode
        }, 2000)
    }

    const goToInvalidField = () => {
        if (data) {
            if (data.status === 409) highlightLabel(1, 'email')
            if (data.status === 410) highlightLabel(2, 'phoneNumber')
            if (data.status === 411) highlightLabel(2, 'ssn')
        }
    }

    const closeAlert = () => {
        document.querySelector('.overlay').classList.remove('open')
        // check if there is error and navigate to the page containig the invalid field
        goToInvalidField()
    }
    ...
    }, [formState, validPhoneNumber, validSsn])
            ...
            <div
                className={
                    showForm === 'registration'
                        ? 'login-inner registration-form'
                        : 'login-inner registration-form hide'
                }
            >
                <h1>Sign Up</h1>
                <p>Create an account to continue.</p>

                <form className="registration-middle" onSubmit={handleSubmit}>
                    <div
                        className={stepNumber === 1 ? 'step' : 'step hideLeft'}
                    >
                        {pageOne.map((item) => {
                            return (
                        ...
                        <div className="input">
                            ...
                            <div className="inputInner">
                                ...
                                <input
                                    ...
                                    name={pageFive.name}
                                    id={pageFive.name}
                                    placeholder={pageFive.placeholder}
                                    onChange={handleChange}
                                    ref={pageFive.ref}
                                />
                            </div>
                    ...
                    {stepNumber === 1 && (
                        <div className="next-btn" onClick={nextStep}>
                            <p>Next</p>
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="14"
                    ...
                    {stepNumber !== 1 && stepNumber !== 5 && (
                        <div className="buttons">
                            <div className="prev-btn" onClick={prevStep}>
                                <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    width="14"
                                    height="14"
